            ...
            <tbody>
                ...
                @foreach ($ros as $ro)
                    <tr class="bg-white">
                        ...
                        <td class="border border-gray-300 p-2">{{ $ro->kode }}</td>
                        <td class="border border-gray-300 p-2">{{ $ro->ro }}</td>
                        <td class="border border-gray-300 p-2 text-center">
                            <button wire:click="openModal({{ $ro->id }}, 'komponen')" class="bg-blue-500 text-white px-2 py-1 rounded">Tambah</button>
                        </td>
                    </tr>
                    ...
                    @if ($expanded['ro'][$ro->id] ?? false)
                        @foreach ($ro->komponens as $komponen)
                            <tr class="bg-gray-100">
                                ...
                                <td class="border border-gray-300 p-2">{{ $komponen->kode }}</td>
                                <td class="border border-gray-300 p-2">{{ $komponen->komponen }}</td>
                                <td class="border border-gray-300 p-2 text-center">
                                    <button wire:click="openModal({{ $komponen->id }}, 'subKomponen')" class="bg-green-500 text-white px-2 py-1 rounded">Tambah</button>
                                </td>
                            </tr>
                            ...
                            @if ($expanded['komponen'][$komponen->id] ?? false)
                                @foreach ($komponen->subKomponens as $subKomponen)
                                    <tr class="bg-gray-200">
                                        ...
                                        <td class="border border-gray-300 p-2">{{ $subKomponen->kode }}</td>
                                        <td class="border border-gray-300 p-2">{{ $subKomponen->sub_komponen }}</td>
                                        <td class="border border-gray-300 p-2 text-center">
                                            <button wire:click="openModal({{ $subKomponen->id }}, 'detil')" class="bg-yellow-500 text-white px-2 py-1 rounded">Tambah</button>
                                        </td>
                                    </tr>
                                    ...
                                    @if ($expanded['subKomponen'][$subKomponen->id] ?? false)
                                        @foreach ($subKomponen->detils as $detil)
                                            <tr class="bg-gray-300">
                                                ...
                                                <td class="border border-gray-300 p-2">{{ $detil->kode }}</td>
                                                <td class="border border-gray-300 p-2">{{ $detil->detil }}</td>
                                                <td class="border border-gray-300 p-2 text-center">
                                                    <button wire:click="openModal({{ $detil->id }}, 'subDetil')" class="bg-red-500 text-white px-2 py-1 rounded">Tambah</button>
                                                </td>
                                            </tr>
                                            ...
                                            @if ($expanded['detil'][$detil->id] ?? false)
                                                @foreach ($detil->subDetils as $subDetil)
                                                    <tr class="bg-gray-400">
                                                        ...
                                                        <td class="border border-gray-300 p-2"></td>
                                                        <td class="border border-gray-300 p-2">{{ $subDetil->sub_detil }}</td>
                                                        <td class="border border-gray-300 p-2"></td>
                                                    </tr>
                                                @endforeach
                                            @endif
            ...
            <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
                <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                    ...
                    <div class="flex justify-end space-x-2">
                        ...
                        <button wire:click="saveData" class="bg-blue-600 text-white px-4 py-2 rounded">Simpan</button>
                    </div>
                </div>
            </div>
        @endif
